...
export default {
  ...
  methods: {
    enviarTexto(inputText)
    {
      ...
      fetch("https://cyclops.uab.cat/api/session/" + this.$route.params.sessionId + "/callEventWS/" + "-1" + "/"+ inputText,
      {
        method: 'GET',
        headers: { 
    ...
    getEvents() {
      fetch("https://cyclops.uab.cat/api/session/" + this.$route.params.sessionId + "/available-eventsWS")
      .then(response => response.json())
      .then(retrievedData => {
    ...
    sendEvent(eventId) {
      ...
      fetch("https://cyclops.uab.cat/api/session/" + this.$route.params.sessionId + "/callEventWS/" + eventId,
      {
        method: 'GET',
        headers: { 
          'Content-Type': 'application/json',
